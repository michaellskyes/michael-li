...
//borrwoed code from class 3 "B_lotsofsquares"
int cols = 50;
int rows = 50;
...
void draw()
{
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      {
        c[1-which].pixels[i*w + j] = color(167, 255, 255);
      } else if (center==0 && sum==2) // dead... become alive
      {
        c[1-which].pixels[i*w + j] = color(255, 168, 255);
